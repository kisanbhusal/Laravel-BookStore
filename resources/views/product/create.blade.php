...
<hr class="h-1 bg-blue-200">
  <form action="{{ route('product.store') }}" class="mt-5" method="POST" enctype="multipart/form-data">
    @csrf
    ...
    <p class="text-red-600" text-xs -mt-2">{{ $message }}</p>
      ...
      <input type="text" placeholder="Language" name="language" class="w-full rounded-lg border-gray-300 my-2" value="{{ old('language') }}">
      @error('language')
      <p class="text-red-600" text-xs -mt-2">{{ $message }}</p>
        @enderror
...
<input type="number" placeholder="Old price" name="oldprice" class="w-full rounded-lg border-gray-300 my-2" value="{{ old('oldprice') }}">
    ...
    <p class="text-red-600" text-xs -mt-2">{{ $message }}</p>
      ...
      <textarea placeholder="Description" name="description" class="w-full  row-5 rounded-lg border-gray-300 my-2" value="">{{ old('description') }}</textarea>
    @error('description')
    <p class="text-red-600" text-xs -mt-2">{{ $message }}</p>
      @enderror
    
    <div class="flex">
  <input type="submit" class="bg-blue-600 text-white px-2 py-1 rounded shadow hover:shadow-blue-400" value="Add Product">
